...
def set_wallpaper(image_path):
    ...
    # Escape for AppleScript: double any backslashes and escape double quotes
    safe_path = abs_path.replace('\\', '\\\\').replace('"', '\\"')

    # Use the Finder approach with POSIX file — this is the most reliable method
    script = f'''
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logging.warning(f"[Wallpaper] Finder script failed: {result.stderr.strip()}")
    except Exception as e:
        logging.error(f"[Wallpaper] Error: {e}", exc_info=True)
