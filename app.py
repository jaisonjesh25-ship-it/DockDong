...
class AppDelegate(NSObject):
    def applicationDidFinishLaunching_(self, notification):
        ...
        # Use NSRunLoopCommonModes so the timer keeps firing even when the
        # user is interacting with other apps or the system is in a modal
        # tracking loop (the default mode pauses timers in that situation).
        ...
        NSRunLoop.currentRunLoop().addTimer_forMode_(self.timer, Foundation.NSRunLoopCommonModes)
    ...
    def backend_worker(self):
        try:
            ...
            logging.error(f"Update failed with error: {error_msg}")
